  ...
  return (
    <div className="relative flex h-14 w-14 items-center justify-center">
      <svg className="-rotate-90" width="56" height="56" viewBox="0 0 56 56">
        <circle cx="28" cy="28" r={r} fill="none" strokeWidth="3" className="stroke-white/8" />
        <circle
...
function RiskSignalCard({ signal }: { signal: RiskSignal }) {
  const badgeClass = SEVERITY_BG[signal.severity] ?? "bg-white/5 text-white/40 border-white/10";
    ...
    <div className="glass-panel rounded-2xl p-5 space-y-3">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-0.5">
          ...
          {signal.headline && (
            <p className="text-sm font-semibold leading-snug text-white/85 mt-1">
              {signal.headline}
            </p>
          )}
        </div>
        ...
        <div className="shrink-0 text-right">
          <span className="font-mono text-lg font-bold text-white/80">{pct}%</span>
          <p className="text-[10px] text-white/30">confident</p>
        </div>
      </div>
      ...
      {/* Explanation */}
      {signal.explanation && (
        <p className="text-[12px] leading-relaxed text-white/55">{signal.explanation}</p>
      )}

      {/* Evidence summary */}
      {signal.evidence_summary && (
        <div className="rounded-lg border border-white/6 bg-white/3 px-3 py-2">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-white/25 mb-1">Evidence</p>
          <p className="text-[11px] leading-relaxed text-white/50">{signal.evidence_summary}</p>
        </div>
      )}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {signal.product_principle_refs.map((ref: ProductPrincipleRef) => (
              <span
                key={ref.id}
                className="flex items-center gap-1 rounded-full border border-violet-400/20 bg-violet-400/8 px-2.5 py-1 text-[10px] text-violet-300/70"
                title={ref.excerpt ?? undefined}
              >
                <BookOpen className="h-3 w-3 shrink-0" />
                {ref.source && <span className="text-violet-300/50">{ref.source}:</span>}
                {ref.name}
              </span>
            ))}
          </div>
          ...
          {signal.product_principle_refs[0]?.excerpt && (
            <p className="text-[11px] leading-relaxed text-white/35 italic pl-1">
...
function MetricCard({ name, target, current, unit }: {
  ...
  return (
    <div className="rounded-xl border border-white/6 bg-white/3 px-3 py-2.5">
      <p className="text-[10px] font-medium uppercase tracking-widest text-white/30 truncate">{name}</p>
      <div className="mt-1 flex items-baseline gap-1.5">
        {current != null ? (
          <>
            <span className="font-mono text-base font-bold text-white/80">{current}</span>
            <span className="text-[11px] text-white/30">/ {target} {unit}</span>
          </>
        ) : (
          <span className="font-mono text-base font-bold text-white/40">{target} {unit}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="relative pl-7">
      <div className={cn(
        ...
        isPrimary ? "border-indigo-500/50 bg-indigo-500/10" : "border-white/10 bg-white/3",
      )}>
        <div className={cn("h-1.5 w-1.5 rounded-full", isPrimary ? "bg-indigo-400" : "bg-white/20")} />
      </div>
      <div className="space-y-0.5">
        <div className="flex items-center gap-2">
          <span className={cn(
            "text-[10px] font-semibold uppercase tracking-wide",
            isPrimary ? "text-emerald-400/80" : "text-white/30",
          )}>
            {intervention.status === "accepted" ? "Approved" : intervention.status}
          </span>
          <span className="text-[10px] text-white/25">·</span>
          <span className="text-[10px] text-white/40">{ACTION_LABELS[intervention.action_type]}</span>
        </div>
        <p className="text-[11px] leading-relaxed text-white/40 line-clamp-2">{intervention.rationale}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <Suspense fallback={
      <div className="flex flex-col gap-4 pb-6 animate-pulse">
        <div className="h-8 w-48 rounded-xl bg-white/6" />
  ...
  if (betError || !bet) {
    return (
      <div className="flex flex-col items-center gap-3 py-20 text-center">
        ...
        <p className="text-sm text-white/50">Direction not found or backend unavailable.</p>
        <Link href="/workspace/directions" className="text-xs text-indigo-400 underline hover:text-indigo-300">
          Back to Directions
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-5 pb-8">
      ...
      <Link
        ...
        className="flex w-fit items-center gap-1.5 text-xs text-white/35 transition-colors hover:text-white/65"
      >
        <ArrowLeft size={13} />
        All directions
      </Link>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <h1 className="font-heading text-xl font-bold text-white/90">{bet.name}</h1>
              <span className={cn(
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[12px] text-white/40">
              {bet.target_segment && (
                <span className="flex items-center gap-1">
                  <Target size={11} className="text-sky-400/50" />
                  {bet.target_segment}
                </span>
              )}
              {bet.time_horizon && (
                <span className="flex items-center gap-1">
                  <Clock size={11} className="text-amber-400/50" />
                  {bet.time_horizon}
                </span>
              )}
              {bet.last_monitored_at && (
                <span className="flex items-center gap-1">
                  <Zap size={11} className="text-indigo-400/50" />
      ...
      <div className="grid grid-cols-12 gap-5">
        <div className="col-span-7 glass-panel rounded-2xl p-5 space-y-3">
          <h2 className="font-heading text-sm font-semibold text-white/70 flex items-center gap-2">
            <MessageSquare size={14} className="text-indigo-400/60" />
            Hypothesis
          </h2>
          {bet.hypothesis ? (
            <p className="text-[12px] leading-relaxed text-white/55">{bet.hypothesis}</p>
          ) : (
            <div className="flex items-center gap-2 rounded-lg border border-amber-400/15 bg-amber-400/5 px-3 py-2">
              <AlertTriangle size={13} className="text-amber-400/70 shrink-0" />
        ...
        <div className="col-span-5 glass-panel rounded-2xl p-5 space-y-3">
          <h2 className="font-heading text-sm font-semibold text-white/70 flex items-center gap-2">
            <BarChart2 size={14} className="text-sky-400/60" />
            Success metrics
          </h2>
          {(bet.success_metrics?.length ?? 0) > 0 ? (
            ...
            <p className="text-[11px] text-white/30">No metrics defined yet.</p>
          )}
        </div>
      </div>
      ...
      {pending[0]?.risk_signal && (
        <div className="space-y-2">
          <h2 className="font-heading text-sm font-semibold text-white/70 flex items-center gap-2">
            <Zap size={14} className="text-indigo-400/60" />
            Current risk signal
          </h2>
      ...
      {resolved.length > 0 && (
        <div className="glass-panel rounded-2xl p-5 space-y-4">
          ...
          <div className="relative space-y-5">
            <div className="absolute bottom-2 left-[7px] top-2 w-px bg-white/6" />
            {resolved.map((i) => (
      ...
      {!loadingInterventions && interventions.length === 0 && (
        <div className="glass-panel flex flex-col items-center gap-2 rounded-2xl py-12 text-center">
          ...
          <p className="text-sm font-medium text-white/50">No interventions yet</p>
          <p className="text-xs text-white/30">Aegis will flag risks after the next scan runs.</p>
        </div>
      )}
    </div>
  );
